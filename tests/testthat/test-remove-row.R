...
test_reactive_vals$click_data <- data.frame(x_values = c(1, 2, 3, 4),
                                             y_values = c(5, 6, 7, 8))
...
test_that("Removing a row works", {
  expect_equal(shiny::isolate({remove_last_row(test_reactive_vals)$click_data}),
               test_one_removed_data)
})

test_reactive_vals2 <- shiny::reactiveValues()
test_reactive_vals2$click_data <- data.frame(x_values = c(1, 2, 3, 4),
                                            y_values = c(5, 6, 7, 8))
...
test_that("Repeatedly removing a row works", {
  expect_equal(shiny::isolate({remove_last_row(remove_last_row(test_reactive_vals2))$click_data}),
