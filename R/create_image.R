...
#'
create_image <- function(loaded_image, effects, click_data, point_size, colour_select) {

  displayed_image <- apply_effects(effects, loaded_image)
  displayed_image <- magick::image_ggplot(displayed_image)
  displayed_image <- displayed_image + geom_point(data = click_data, aes(x = .data$x_values,
                                                       y = .data$y_values),
